...
public class DatabaseIncreaseGamesTableAfterLoseTest extends SetupTests {

    private final Integer[] money = { 5, 5 };
    private final Integer difficulty = 3;
    private final Integer[] position = { 1, 7, 5 };
	...
	final int expected = getRowsCount(BundleUtils.QUERY.getString("countGamesTable")) + 1;
	final String[] newUserName = { UUID.randomUUID().toString() };
	setList(money, difficulty, position, newUserName);
	machine = machineBuilder();
	final int count = getRowsCount(BundleUtils.QUERY.getString("countGamesTable"));
